...
def copy_role_for_spec
  copy_dest = "spec/roles/openstack_keystone_database"
...
def clone_mariadb_role_for_spec
  ...
  delete_targets = Dir.glob(copy_dest)
  FileUtils.rm_rf(delete_targets)
  system("cd spec/roles && git clone https://github.com/bbrfkr/ansible_role_openstack_mariadb.git bbrfkr.openstack_mariadb")
  system("cd spec/roles/bbrfkr.openstack_mariadb && git checkout ocata -b origin/ocata")
end
